...
export class SymbolReplacer {
    public static readonly replacePatterns: Array<ReplacePattern> = [
        { from: /^БЮ$/, to: "<>" },
        { from: /^!=$/, to: "<>" },
    ...
    public replaceSpecSymbols(event: TextDocumentChangeEvent): void {
        ...
        const text = editor.document.getText(new Range(new Position(originalPosition.line, 0), originalPosition));
        ...
        SymbolReplacer.replacePatterns.forEach((p) => {
            ...
            editor.edit((builder) => {
                const contentChangeRange = new Range(
                    p.removeFirstCount ? startPosition.translate(0, -p.removeFirstCount) : startPosition,
